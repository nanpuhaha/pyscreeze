...
def _locateAll_opencv(
    needleImage, haystackImage, grayscale=None, limit=10000, region=None, step=1, confidence=0.999
):
    """
    TODO - rewrite this
...
def _locateAll_python(
    needleImage, haystackImage, grayscale=None, limit=None, region=None, step=1, confidence=None
):
    """
    TODO
    """
    ...
    assert [len(row) for row in needleImageRows] == [
        ...
        "For some reason, the needleImageRows aren't the same size as the original image."
    )
...
def locateOnWindow(image, title, **kwargs):
    ...
    if _PYGETWINDOW_UNAVAILABLE:
        raise PyScreezeException(
            "locateOnWindow() failed because PyGetWindow is not installed or is unsupported on this platform."
        )
